...
const callGeminiAPI = async (prompt, retries = 5) => {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      return data.candidates?.[0]?.content?.parts?.[0]?.text || "暫時無法分析，請稍後再試。";
    } catch (error) {
      if (i === retries - 1) return "AI 伺服器忙碌中，請稍後嘗試。";
...
const App = () => {
  ...
  // 資料抓取邏輯：依賴 currentStock，移除隨機模擬價格
  useEffect(() => {
    let isMounted = true;
        ...
        if (isMounted) {
          setAppData({
            macro: { 
                ...
                vix: 14 + (hash % 8),
                news: [
                    { source: 'Reuters', text: `$${currentStock} 精確雲端數據同步成功。` },
                    { source: '系統日誌', text: `已從交易所抓取 ${currentStock} 最新成交資訊與技術因子。` }
                ] 
            },
        ...
        if (isMounted) {
          setAppData({
            macro: { 
              ...
              fearGreedIndex: 30, 
              news: [{ source: '連線提示', text: '後端引擎正在喚醒中（約30秒），請稍後搜尋重新同步真實數據。' }] 
            },
  ...
  if (isLoading || !appData) return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-blue-400">
      ...
      <div className="text-sm text-slate-500 mt-4 flex items-center">
        <Loader2 className="w-4 h-4 mr-2 animate-spin" /> 正在與雲端同步 {currentStock} 即時行情...
      </div>
    </div>
  );
        ...
        <div className="max-w-[1700px] mx-auto flex justify-between items-center gap-4">
          <div className="flex items-center space-x-3">
            ...
            {appData.isReal ? 
              ...
              <span className="bg-orange-500/10 text-orange-400 text-[10px] px-3 py-1 rounded-full border border-orange-500/20">喚醒/模擬模式</span>
            }
          </div>

          <form onSubmit={handleSearch} className="flex-1 max-w-xl relative group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-blue-500 transition-colors" />
            <input type="text" value={tickerInput} onChange={(e)=>setTickerInput(e.target.value)} placeholder="搜尋代碼 (例: 2330, NVDA)..." className="w-full bg-slate-950 border border-slate-700 text-white rounded-2xl pl-10 pr-4 py-2 text-sm focus:border-blue-500 outline-none uppercase transition-all" />
          </form>

          <div className="flex items-center space-x-4">
             <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700">
                <button onClick={()=>setCurrentStock('AAPL')} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${market==='US' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>US</button>
                <button onClick={()=>setCurrentStock('2330')} className={`px-4 py-1.5 rounded-lg text-xs font-black transition-all ${market==='TW' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>TW</button>
             </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl">
               ...
               <div className="grid grid-cols-3 gap-2 mb-6 text-center">
                  <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800"><span className="text-[9px] text-slate-500 block mb-1">S&P 500</span><span className="text-xs font-black text-white font-mono">{appData.macro.indices.sp500.toFixed(1)}</span></div>
                  <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800"><span className="text-[9px] text-slate-500 block mb-1">加權指數</span><span className="text-xs font-black text-red-400 font-mono">{appData.macro.indices.taiex.toFixed(0)}</span></div>
                  <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800"><span className="text-[9px] text-slate-500 block mb-1">USD/TWD</span><span className="text-xs font-black text-white font-mono">{appData.macro.indices.usdtwd.toFixed(2)}</span></div>
               </div>
               ...
               <div className="space-y-3">
                  <span className="text-purple-300 font-black text-[10px] flex items-center uppercase tracking-widest"><Sparkles className="w-3.5 h-3.5 mr-2"/> AI NLP Feed</span>
            ...
            <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl flex flex-col min-h-[550px]">
               ...
               <div className="flex-1 bg-slate-950 rounded-[2rem] mb-6 relative overflow-hidden group border border-slate-800 shadow-inner">
                  <svg className="w-full h-full p-8 relative z-10" preserveAspectRatio="none" viewBox="0 0 100 100">
                    <line x1="0" y1="40" x2="100" y2="40" stroke="#3b82f6" strokeWidth="0.5" strokeDasharray="5,5" opacity="0.4" />
                  ...
                  <div className="absolute bottom-6 right-6 bg-slate-900/90 border border-slate-700 px-4 py-2 rounded-2xl text-[11px] font-black font-mono text-cyan-400 shadow-2xl">
               ...
               {appData.tech.chipData?.isTWSE && appData.isReal && (
                <div className="mb-6 space-y-4">
                   <div className="grid grid-cols-3 gap-4 bg-blue-900/10 border border-blue-500/20 p-6 rounded-3xl shadow-xl">
                      <div className="flex flex-col"><span className="text-[10px] text-slate-500 uppercase font-black mb-1">外資 (張)</span><span className={`font-mono font-black text-2xl ${appData.tech.chipData.foreignInvestor >= 0 ? 'text-red-400' : 'text-green-400'}`}>{appData.tech.chipData.foreignInvestor > 0 ? '+' : ''}{appData.tech.chipData.foreignInvestor}</span></div>
                      <div className="flex flex-col border-x border-slate-800 px-5"><span className="text-[10px] text-slate-500 uppercase font-black mb-1">投信 (張)</span><span className={`font-mono font-black text-2xl ${appData.tech.chipData.investmentTrust >= 0 ? 'text-red-400' : 'text-green-400'}`}>{appData.tech.chipData.investmentTrust > 0 ? '+' : ''}{appData.tech.chipData.investmentTrust}</span></div>
                      <div className="flex flex-col"><span className="text-[10px] text-slate-500 uppercase font-black mb-1">融資維持率</span><span className={`font-mono font-black text-2xl ${appData.tech.marginRate < 130 ? 'text-red-500 animate-pulse' : 'text-white'}`}>{appData.tech.marginRate}%</span></div>
                   </div>
                   {appData.tech.marginRate < 130 && (
                     <div className="bg-red-950/40 border border-red-500/50 p-5 rounded-3xl flex items-center animate-pulse">
                        <AlertTriangle className="w-6 h-6 text-red-500 mr-4" />
                        <span className="text-red-300 text-xs font-black uppercase tracking-tight">警告：融資維持率低於 130%，嚴防多殺多賣壓。</span>
                     </div>
                   )}
                </div>
               )}
               ...
               <div className="grid grid-cols-4 gap-4">
                 {['KD', 'RSI', 'MACD', 'VOL'].map(ind => (
                   <button key={ind} onClick={()=>setActiveIndicator(ind)} className={`py-3.5 rounded-2xl border text-[11px] font-black transition-all shadow-md ${activeIndicator===ind ? 'bg-blue-600 border-blue-500 text-white shadow-lg' : 'bg-slate-950 border-slate-800 text-slate-500 hover:border-slate-600'}`}>{ind}</button>
                 ))}
               </div>
            </div>
            ...
            <div className="bg-slate-900 p-6 rounded-[2.5rem] border border-slate-800 shadow-2xl">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div onClick={async()=>{
                    ...
                    }} className="bg-slate-950 p-6 rounded-[2rem] border border-emerald-500/20 hover:border-emerald-500/60 cursor-pointer transition-all group">
                    <h4 className="text-emerald-400 font-black text-xs uppercase mb-2 flex items-center tracking-widest"><DollarSign className="w-4 h-4 mr-2"/> 價值建倉戰略</h4>
                    <p className="text-[11px] text-slate-500 leading-relaxed font-medium">當前估值具備安全邊際，適合左側分批佈局核心部位...</p>
                  </div>
                  
                  <div onClick={async()=>{
                      setIsDeepDiving(true);
                      setSelectedStrategy({title: '波段作多操作計畫'});
                      const res = await callGeminiAPI(`為 ${currentStock} 規劃波段計畫。當前價: ${appData.stock.price}。`);
                      setAiDeepDiveResult(res);
                      setIsDeepDiving(false);
                    ...
                    <h4 className="text-purple-400 font-black text-xs uppercase mb-2 flex items-center tracking-widest"><TrendingUp className="w-4 h-4 mr-2"/> 波段作多戰略</h4>
                    <p className="text-[11px] text-slate-500 leading-relaxed font-medium">指標底部分離修正，量能溫和放大，建議設定關鍵支撐後進場...</p>
                  </div>

                  {/* 當沖高風險紅標卡片 */}
                  <div onClick={()=>setSelectedStrategy({title: '當沖交易警告', details: 'VIX 波動率偏高，目前量化期望值為負。紀律要求：收盤前絕對平倉，絕不留倉。'})} className="md:col-span-2 bg-red-950/20 border border-red-900/40 p-5 rounded-[2rem] flex items-center hover:bg-red-900/30 transition-all cursor-pointer group shadow-lg">
                     ...
                     <div>
                        ...
                        <p className="text-[10px] text-red-300/70 font-medium">根據量化引擎試算，目前市場期望值為負。戰術要求：<span className="text-red-400 underline font-black">收盤絕對不留倉。</span></p>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div className="p-8 text-sm text-slate-300 leading-relaxed max-h-[60vh] overflow-y-auto">
               {isDeepDiving ? (
                 <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="text-slate-500 font-black uppercase tracking-[0.2em] text-[10px]">Computing Strategy...</p>
                 </div>
               ) : (
