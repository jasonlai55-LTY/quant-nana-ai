    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isMounted) {
          setAppData({
            macro: macroFallback,
            stock: { price: basePrice, change: "1.23", pe: 15.5, roe: 18.2, institutionalHoldings: 65, analystTarget: 180, isSuitableForDCA: true, dcaReason: "範例原因" },
      ...
      <main className="flex-1 p-4 max-w-[1800px] mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-10 grid grid-cols-1 lg:grid-cols-3 gap-5">
           ...
           <div className="lg:col-span-2 bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col">
              ...
              <div className="flex-1 bg-slate-950 rounded-lg mb-4 flex items-center justify-center text-slate-700 italic text-sm">
                 [雲端連線成功，正在接收來自 Render 的 K 線串流資料...]
              </div>
              <div className="grid grid-cols-4 gap-2 text-[10px]">
                 <div className="bg-slate-950 p-2 rounded border border-slate-800">RSI: <span className="text-white">{appData.tech.rsi}</span></div>
                 <div className="bg-slate-950 p-2 rounded border border-slate-800">KD: <span className="text-white">{appData.tech.kd.k}/{appData.tech.kd.d}</span></div>
                 <div className="bg-slate-950 p-2 rounded border border-slate-800">MACD: <span className="text-white">{appData.tech.macd}</span></div>
                 <div className="bg-slate-950 p-2 rounded border border-slate-800">斷頭位: <span className="text-red-400">{appData.tech.marginRate}%</span></div>
              </div>
           </div>
        </div>
      </main>
